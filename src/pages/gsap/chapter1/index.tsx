...
export default function Gsap() {
  ...
  useEffect(() => {
    animationRef.current = gsap.to("#tiger", {
      duration: 6,
      motionPath: {
        path: "#path",
        align: "#path",
      ...
      //   onUpdate: () => {
      //     // onUpdate 지속적인 호출 => 애니메이션이 재생될때,
      //     console.log("playing");
      //   },
      // 함수로 대체해서 쓸수있다 update ,
      onUpdate: update,
      onComplete: () => {
        console.log("끝");
      },
  ...
  const update = () => {
    setSliderValue(animationRef.current?.time());
  };
  ...
  const ClickMountain = () => {
    gsap.to(animationRef.current, { progress: 0.2, duration: 5 });
    animationRef.current?.pause();
  };

  const ClickRiver = () => {
    gsap.to(animationRef.current, { progress: 0.48, duration: 5 });
    animationRef.current?.pause();
  };

  const ClickBuilding = () => {
    gsap.to(animationRef.current, { progress: 1, duration: 5 });
    animationRef.current?.pause();
  };
    ...
    <Wrapper>
      <svg
        ...
        <g id="svg">
          ...
          <g id="company" onClick={ClickBuilding} style={{ cursor: "pointer" }}>
            <rect
              width="60"
              height="67"
          ...
          <g id="river" onClick={ClickRiver} style={{ cursor: "pointer" }}>
            <rect
              width="54.5"
              height="63"
      ...
      <div className="indicator">
        ...
        <input
          ...
          value={sliderValue}
          step="0.1"
          onChange={(e: any) => setSliderValue(parseFloat(e.target.value))}
        />
        <div id="progress">{sliderValue?.toFixed(2) || 0}</div>
      </div>
    </Wrapper>
  );
}
